...
from shared.events import GamesandMinutesEvents
...
class GameAPI:
    ...
    def evapi(self):
        # Import EventAPI locally to avoid circular imports
        from feedapi.event_api import EventAPI
        if self.event_api_connection is False:
            self.event_api_connection = EventAPI(competition_id=self.competitionID, season_id=self.seasonID)
            return self.event_api_connection
        else:
            return self.event_api_connection

    def papi(self):
        # TODO: In microservices, this should call player-service API instead
        # Temporarily disabled - cross-service dependencies need HTTP calls
        raise NotImplementedError("PlayerAPI now in player-service - use HTTP API instead")

    def tapi(self):
        # TODO: In microservices, this should call team-service API instead
        # Temporarily disabled - cross-service dependencies need HTTP calls
        raise NotImplementedError("TeamAPI now in team-service - use HTTP API instead")
