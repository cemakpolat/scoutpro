...
class PlayerRepository(BaseRepository[ScoutProPlayer]):
    ...
    def to_document(self, player: ScoutProPlayer) -> Dict[str, Any]:
        ...
        doc = {
            '_id': player.id,
            'scoutpro_id': player.id,
            'uID': (player.provider_ids or {}).get('opta') or player.external_id,
            ...
            'height': str(player.height_cm) if player.height_cm is not None else None,
            'weight': str(player.weight_kg) if player.weight_kg is not None else None,
            ...
            'teamName': (player.scoutpro_metadata or {}).get('team_name'),
            'club': (player.scoutpro_metadata or {}).get('team_name'),
    ...
    def from_document(self, doc: Dict[str, Any]) -> ScoutProPlayer:
        ...
        player = ScoutProPlayer(
            ...
            foot=doc.get('foot') or doc.get('preferred_foot'),
